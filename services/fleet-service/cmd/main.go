...
import (
	...
	"github.com/ThatSneakyCoder/RoutePulse/services/fleet-service/internal/infrastructure/db"
	"github.com/ThatSneakyCoder/RoutePulse/services/fleet-service/internal/infrastructure/events"
	"github.com/ThatSneakyCoder/RoutePulse/services/fleet-service/internal/infrastructure/grpc"
...
func main() {
	...
	log.Infow("fleet service starting",
		"env", cfg.env,
	...
	log.Info("database connection established in fleet service")
	...
	publisher := events.NewFleetEventPublisher(rabbitmq)

	// Wiring
	fleetRepo := domain.NewFleetStore(dbConn, log)
	fleetSvc := service.NewFleetService(fleetRepo, log, publisher)

	grpcServer := grpcserver.NewServer()
	grpc.NewGRPCHandler(grpcServer, fleetSvc, log)

	log.Infow("starting fleet gRPC server",
	...
	log.Info("gracefully shutting down fleet gRPC server")
